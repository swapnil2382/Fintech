  ...
  try {
    ...
    const stockSymbols = stockTransactions
      .map((txn) => {
        ...
        if (!match) {
          console.warn(
            `Invalid stock purchase description format: ${txn.description}`
          );
          return null;
        }
        return match[2];
    ...
    res
      .status(500)
      .json({ error: "Error calculating tax", details: error.message });
  }
});
...
router.get("/form", authMiddleware, async (req, res) => {
  try {
    ...
    const taxForm = {
      ...
      name: user.username || "Unknown User", // Fallback if username is missing
      pan: "ABCDE1234F",
      bankAccountName: bankAccount.name,
      totalIncome: totalIncome.toFixed(2),
      incomeFromSalary: totalIncomeFromBank.toFixed(2),
      incomeFromCapitalGains: capitalGains.toFixed(2),
    ...
    res
      .status(500)
      .json({ error: "Error generating tax form", details: error.message });
  }
});
